#!/usr/bin/env python
from __future__ import print_function# 让python2.x像3一样使用
import argparse
import os
import time
import numpy as np
...
class Processor():
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.train_feeder_args), # 传入一系列train参数
        ...
        self.data_loader['test'] = torch.utils.data.DataLoader(
            dataset=Feeder(**self.arg.test_feeder_args),# 传入一系列test参数
    ...
    def load_model(self):
        ...
        Model = import_class(self.arg.model) # 载入模型
        """
        功能：复制文件，保留元数据
...
if __name__ == '__main__':
    ...
    if p.config is not None:
        with open(p.config, 'r') as f:
            default_arg = yaml.load(f) # 把parser里面所有参数要
                                       # 在config中使用yaml样式保存
        key = vars(p).keys()
        for k in default_arg.keys():
            if k not in key: # wrong arg
