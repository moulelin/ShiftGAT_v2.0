...
class Shift_gcn(nn.Module):
    ...
    def forward(self, x0):
        ...
        attentions = [GraphAttentionLayer(n*t,c, (int(c/2)+1), dropout=0.6, alpha=0.3, concat=True) for _ in range(1)]
        for i, attention in enumerate(attentions):
            self.add_module('attention_{}'.format(i), attention)
            
        out_att = GraphAttentionLayer(n*t,(int(c/2)+1)*1, c, dropout=0.6, alpha=0.3, concat=False)
      
        x_att = torch.cat([att(x, torch.ones(v,v).cuda()) for att in attentions], dim=2)
        out_att = out_att(x_att,torch.ones(v,v).cuda())
        
        x = out_att.view(x.size())
